 ...
 * {@link Character#codePointOf(String) Character.codePointOf(name)}.
 * <p>
...
public final class Pattern
     ...
     * Form.NFC NFC}, canonical decomposition,
     * followed by canonical composition for the character class
     * part, and {@link Form.NFD NFD},
    ...
    private static void normalizeSlice(String src, int off, int limit,
        ...
        while (off < limit) {
            ...
            String nfd = Normalizer.normalize(seq, Form.NFD);
            off = j;
            if (nfd.length() > 1) {
            ...
            String nfc = Normalizer.normalize(seq, Form.NFC);
    ...
    private static int codePointOf(String name) {
        ...
        try {
            ...
            if (off != -1) {
                cp = Integer.parseInt(name.substring(off + 1), 16);
                if (Character.isValidCodePoint(cp) && name.equals(Character.getName(cp)))
                    return cp;
            }
        } catch (Exception x) {
        }
